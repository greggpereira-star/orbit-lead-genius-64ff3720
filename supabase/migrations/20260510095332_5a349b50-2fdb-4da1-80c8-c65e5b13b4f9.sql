-- Update create_form_with_fields to be more robust
...
BEGIN
    -- Validate tenant
    IF NOT EXISTS (SELECT 1 FROM public.companies WHERE id = p_tenant_id) THEN
        RAISE EXCEPTION 'Company with ID % not found', p_tenant_id;
    END IF;
...
-- Update update_form_with_fields to be more robust
...
BEGIN
    ...
    SELECT settings INTO v_current_settings FROM public.forms WHERE id = p_form_id;
    v_settings := COALESCE(p_form_data->'settings', '{}'::jsonb);
