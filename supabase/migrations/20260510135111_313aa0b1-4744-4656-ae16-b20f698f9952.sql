-- 1. Create a high-performance membership check function
CREATE OR REPLACE FUNCTION public.check_membership_v2(p_company_id uuid)
RETURNS boolean AS $$
BEGIN
  RETURN EXISTS (
...
-- 2. Drop and Recreate save_form_v2 to handle return type change or param changes correctly
...
BEGIN
    ...
    IF v_user_id IS NULL OR NOT public.check_membership_v2(v_form_company_id) THEN
...
USING (
  EXISTS (
    ...
    AND public.check_membership_v2(f.company_id)
  )
);

DROP POLICY IF EXISTS "Users can manage form steps for their forms" ON public.form_steps;
CREATE POLICY "Users can manage form steps for their forms" 
