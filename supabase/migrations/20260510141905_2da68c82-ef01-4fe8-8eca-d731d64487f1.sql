-- 1. Optimized Form Save RPC v5 (CTE Based with internal validation)
...
DECLARE
    ...
    v_fields_count int;
BEGIN
    -- 1. Authorization Check (Pre-validated)
    ...
    -- SECURITY DEFINER context: We must validate permissions manually
    IF v_user_id IS NULL OR NOT EXISTS (
        SELECT 1 FROM public.memberships 
        WHERE user_id = v_user_id AND company_id = v_form_company_id
    ) THEN
        RAISE EXCEPTION 'Unauthorized: User % lacks access to company %', v_user_id, v_form_company_id USING ERRCODE = '42501';
    END IF;

    -- 2. Update Form Metadata (Only if changed)
    UPDATE public.forms
    SET 
    ...
    IF p_steps IS NOT NULL AND jsonb_array_length(p_steps) > 0 THEN
        ...
        AND id NOT IN (SELECT id FROM step_input);
    ELSE
    ...
    -- 4. Sync Fields (The Critical Part for Dropdowns)
    IF p_fields IS NOT NULL AND jsonb_array_length(p_fields) > 0 THEN
        -- Use a CTE to parse the input once
        WITH field_input AS (
            SELECT 
                CASE 
        ...
        upsert_fields AS (
            ...
            ON CONFLICT (id) DO UPDATE SET
                ...
                options = EXCLUDED.options, -- Crucial for dropdown persistence
                sort_order = EXCLUDED.sort_order,
        ...
        -- Identify fields to delete (those NOT in the input array)
        DELETE FROM public.form_fields 
        WHERE form_id = p_form_id 
        AND id NOT IN (SELECT id FROM field_input);
    ...
    RETURN jsonb_build_object(
        ...
        'trace_id', 'db_' || substring(gen_random_uuid()::text from 1 for 8)
    );
END;
